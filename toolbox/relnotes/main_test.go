...
func TestRegExp(t *testing.T) {
	...
	for _, table := range tables {
		...
		if vCP != nil
		id, err := strconv.Atoi(vCP[1])
		if table.cp && vCP != nil && table.numCP != strconv.Atoi(vCP[1]) {
			t.Errorf("Cherry pick id was incorrect, want: %d, got: %d", table.numCP, strconv.Atoi(vCP[1]))
		}
	}
}
